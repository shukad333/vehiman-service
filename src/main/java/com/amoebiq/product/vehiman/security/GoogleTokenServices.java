...
public class GoogleTokenServices implements ResourceServerTokenServices, InitializingBean {
	...
	private static final Logger logger = LogManager.getLogger(GoogleTokenServices.class);
	...
	public OAuth2Authentication loadAuthentication(String accessToken)
		...
		if (!validationResult.isValid()) {
			throw new UnapprovedClientAuthenticationException(
					"The token is not intended to be used for this application.");
		}
	...
	private Authentication getAuthenticationToken(String accessToken) {
		...
		return new UsernamePasswordAuthenticationToken(new GooglePrincipal(new BigInteger(idStr)), null,
				singleton(new SimpleGrantedAuthority("ROLE_USER")));
	}

	private Map<String, ?> getUserInfo(String accessToken) {
		HttpHeaders headers = getHttpHeaders(accessToken);
		logger.error("userInfoUrl {}",userInfoUrl);
